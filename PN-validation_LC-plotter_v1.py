...
# imports the output file from PN_Predictions_2_table.py
df_data = pd.read_csv(
    '/Users/philippnoodt/Jobs_Bewerbungen/IMA/Python/AL_Kunststoff/2019-03-12/results.csv', header=None)

lenDataset = 392   # manual input! lenth of the original dataset: arr_data.shape[0] in PN_Predictions_2_table.py
data = np.asarray(df_data)[:, 1:]
splits = data[0, :-2]
sams = np.round(splits * lenDataset * 0.9)
...
plt.ylim([-0.2, 5.2])
plt.grid()
# fig1.savefig('AUTO_MPG_RMSE_5test5train.png', dpi=300)
